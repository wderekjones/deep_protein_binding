...
def tensorize_smiles_job(smiles, max_degree=5, max_atoms=None):
    ...
    #   as possible (1)
    atom_tensor = np.zeros((n, max_atoms or 1, n_atom_features),dtype=np.float32)
    bond_tensor = np.zeros((n, max_atoms or 1, max_degree or 1, n_bond_features),dtype=np.float32)
    ...
    for mol_ix, s in enumerate(smiles):
        ...
        assert mol is not None, 'Could not parse smiles {}, error: {}'.format(s, sio.getvalue())
        atoms = mol.GetAtoms()
        bonds = mol.GetBonds()
...
def tensorize_smiles(smiles, max_degree=5, max_atoms=None, workers=cpu_count()-1, chunksize=3000, verbose=True):
    ''' Multiprocess implementation of `tensorize_smiles_job`
    ...
    map_function = partial(tensorize_smiles_job, max_degree=max_degree, max_atoms=max_atoms)
    if verbose:
        print('Tensorising molecules in batches...')
        tensor_list = []
        for tensors in tqdm(pool.imap(map_function, smiles_chunks),total=np.ceil(smiles.shape[0]/chunksize)):
...
def clean_data(smiles_df, workers=cpu_count()-1):
    # referring to this code: http://blog.adeel.io/2016/11/06/parallelize-pandas-map-or-apply/
